...
SCRIPT_PATH = REPO_DIR / "train" / "emobank_finetune.py"

EVAL_STEP_RE = re.compile(
    r"^\[step (?P<step>\d+)\] eval_mse=(?P<mse>[-+0-9.eE]+) "
    r"eval_mae=(?P<mae>[-+0-9.eE]+)"
)
EVAL_EPOCH_RE = re.compile(
    r"^\[epoch (?P<epoch>\d+)\] eval_mse=(?P<mse>[-+0-9.eE]+) "
    r"eval_mae=(?P<mae>[-+0-9.eE]+)"
)
...
def parse_metrics(line):
    match = EVAL_STEP_RE.match(line)
    if match:
        return {
            "kind": "step",
            "x": int(match.group("step")),
            "mse": float(match.group("mse")),
            "mae": float(match.group("mae")),
        }
    match = EVAL_EPOCH_RE.match(line)
    if match:
        return {
            "kind": "epoch",
            "x": int(match.group("epoch")),
            "mse": float(match.group("mse")),
            "mae": float(match.group("mae")),
        }
    return None
