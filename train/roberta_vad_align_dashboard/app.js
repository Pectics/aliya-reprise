...
const fields = {
  ...
  weightDecay: document.getElementById("weightDecay"),
  warmupSteps: document.getElementById("warmupSteps"),
  maxLength: document.getElementById("maxLength"),
  evalEverySteps: document.getElementById("evalEverySteps"),
  seed: document.getElementById("seed"),
  numLabels: document.getElementById("numLabels"),
...
const EVAL_SERIES = [
  { key: "mse", label: "mse", color: "#f15b2a", visible: true },
  { key: "mae", label: "mae", color: "#0f6f8c", visible: true },
];
...
function startStream() {
  ...
  source.onmessage = (event) => {
    ...
    if (payload.type === "metrics") {
      ...
      const kind = metrics.kind || "step";
      const xValue = typeof metrics.x === "number" ? metrics.x : metrics.step;
      if (typeof xValue === "number") {
        if (kind === "step") {
          hasStepMetrics = true;
        }
        if (kind === "epoch" && hasStepMetrics) {
          return;
        }
        EVAL_SERIES.forEach((series) => {
          if (metrics[series.key] !== undefined) {
            evalChart.addPoint(series.key, xValue, metrics[series.key]);
          }
        });
      }
...
function buildArgs() {
  ...
  add("--model_name", fields.modelName.value.trim());
  add("--data_path", fields.dataPath.value.trim());
  add("--output_dir", fields.outputDir.value.trim());
  add("--train_split", fields.trainSplit.value.trim());
  add("--eval_split", fields.evalSplit.value.trim());
  add("--epochs", fields.epochs.value);
  ...
  add("--weight_decay", fields.weightDecay.value);
  add("--warmup_steps", fields.warmupSteps.value);
  add("--max_length", fields.maxLength.value);
  add("--eval_every_steps", fields.evalEverySteps.value);
  add("--seed", fields.seed.value);
  add("--num_labels", fields.numLabels.value);
